...
function getHeadersForUrl(url, pipePart) {
    ...
    if (pipePart && pipePart !== 'none') {
        try {
            const cleaned = pipePart.replace(/^pipe=/, '');
            if (cleaned.startsWith('b64:')) {
                const jsonStr = Buffer.from(cleaned.replace(/^b64:/, ''), 'base64').toString('utf-8');
        ...
        } catch (e) { console.error('[Header Parse Error]', e.message); }
    }
    return headers;
}
...
app.all('/proxy', async (req, res) => {
    ...
    console.log(`[Proxy Headers] UA=${(headers['User-Agent']||'').substring(0,30)} Cookie=${headers['Cookie'] ? 'YES('+headers['Cookie'].length+'ch)' : 'NO'} Origin=${headers['Origin']||'none'}`);
    ...
    try {
        ...
        console.log(`[Proxy] ← ${resp.status} ${targetUrl.substring(0, 60)}`);
        ...
        // If upstream returned error, forward it directly
        if (resp.status >= 400) {
            res.setHeader('Content-Type', ct || 'text/plain');
            return res.status(resp.status).send(content);
        }
        ...
        res.status(e.response ? e.response.status : 500).send(e.message);
    }
});
